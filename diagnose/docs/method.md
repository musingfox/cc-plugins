...
10. **HITL bash script**. Last resort. If a human must click, drive _them_ with the HITL template path the entry gave you so the loop is still structured. Captured output feeds back to you.
...
- [ ] **Agent-runnable**: you can run it unattended; a human in the loop only via the HITL template path the entry gave you.

...
assert_contains "$any_fail" "When any shard in the round ended \`QUOTA\` or \`QUOTA-WINDOW\`, do not re-launch that round's other FAIL shards on the same \`\$PI_DISPATCH_CMD\`" \

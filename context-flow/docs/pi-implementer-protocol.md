...
- Write inside `WORK_DIR`, except `REPORT_FILE` and `ESCALATE_FILE`, the only writes allowed outside it; every other path is denied. Never `cd` out or edit the parent checkout. The worker is launched inside `WORK_DIR` (`cf-pi-dispatch.sh` sets `PI_CWD`), so a bare `git` command lands in the worktree; the rule is the second line of defence, not the first.
...
6. **Scope check** — `actual touched ⊆ declared touches_files`; root build/lock manifests (pyproject.toml, uv.lock, package.json, …) and version-only `.claude-plugin/plugin.json` bumps are allowlisted as warnings; other violations → NEEDS_REPLAN `undeclared_file_touched`.

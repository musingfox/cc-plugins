...
Run `/issue <view>` to list a view from the configured project's `dashboard.base`, created by `/obw:pm`, then select one to read its title, status, priority, and markdown body in the same pane. `/issue` with no argument opens the All Tasks view grouped by status (todo, in-progress, blocked, done), with priority sub-headings inside each status; each status heading shows its card count and `done` shows only its count. Other views stay flat lists. `/issue <card>` opens that card directly. The pane reports missing configuration, unavailable CLI output, and missing cards in place without adding vault content to the conversation.

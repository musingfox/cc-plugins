...
describe('running termaid', () => {
  test('a mermaid block is piped to one pinned termaid run', async ($, on) => {
    ...
    expect(uvxRuns(w)[0].argv).toEqual(['uvx', 'termaid@0.9.0', '--width', '80'])
    expect(uvxRuns(w)[0].init).toEqual({ stdin: 'graph LR\nA-->B', timeoutMs: 5000 })
  })

  test('the obsidian read carries no stdin', async ($, on) => {
    const w = world(on, { read: MERMAID_CARD })
    await shown($, w, 'm')
    expect('stdin' in runsOf(w, 'read')[0].init).toBe(false)
    expect(runsOf(w, 'read')[0].init).toEqual({ timeoutMs: 10000 })
  })
  ...
  test('two blocks run in document order', async ($, on) => {
    ...
    await shown($, w, 'm')
    expect(uvxRuns(w)[0].init.stdin.startsWith('graph LR')).toBe(true)
    expect(uvxRuns(w)[1].init.stdin.startsWith('pie')).toBe(true)
  })
  ...
  test('a card without a fence runs only the dashboard lookups and the read', async ($, on) => {
    const w = world(on, { read: CARD })
    await shown($, w, 'mod-obw-issue-pane')
    expect(w.runs.length).toBe(3)
    expect(uvxRuns(w)).toEqual([])
  })
})
...
describe('a diagram after a newer read', () => {
  test('a diagram for another card is dropped', async ($, on) => {
    const w = world(on, { query: AB, read: MERMAID_CARD, termaid: 'defer' })
    await shown($, w, 'a')
    await shown($, w, 'b')
    w.release(3, 'DIAGRAM-A')
    await w.clock.settle()

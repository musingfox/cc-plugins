...
test('refuses a bad vault before a bad project on a query', () => expect(baseQueryArgv({ vault: '', project: '' }, 'Active')).toEqual({ refused: 'vault' }))
...
test('refuses a bad vault before a bad project on a read', () => expect(cardPathArgv({ vault: '', project: '' }, 'pm/cc-plugins/tasks/a.md')).toEqual({ refused: 'vault' }))

...
test('never emits an empty or file target', () => { for (const result of [listArgv('obsidian', 'cc-plugins'), cardArgv('obsidian', 'cc-plugins', 'mod-obw-issue-pane'), cardArgv('obsidian', 'cc-plugins', 'my card')]) if ('argv' in result) for (const arg of result.argv) { expect(arg.startsWith('file=')).toBe(false); expect(arg).not.toBe('path='); expect(arg.endsWith('/.md')).toBe(false) } })

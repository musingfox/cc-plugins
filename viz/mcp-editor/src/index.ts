...
async function startHTTPServer(html: string): Promise<ServerInfo> {
  let resolvePromise: (content: string) => void;
  const waitForDone = new Promise<string>((resolve) => {
...
server.registerTool(
  ...
  async ({ content, title }) => {
    try {
      ...
      return {
        content: [{ type: 'text' as const, text: editedContent }],
      };
    } catch (error) {

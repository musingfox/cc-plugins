...
- **`scripts/pi-probe.sh [--bin-only] [PROBE_DIR]`** — pre-flight gate: `--bin-only` checks the agent binary is on PATH (exit 0/1); the full probe runs `say ok` on the exact routing a dispatch would resolve, bounded by `PI_PROBE_DEADLINE_S` (default 60) and reporting `STALLED` rather than hanging its caller. Callers never touch the agent binary themselves.
